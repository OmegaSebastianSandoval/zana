        ...
        <div class="content-dashboard">
            <div class="row">
				<div class="col-3">
		            ...
		            <input type="text" class="form-control" name="plan_nombre" value="<?php echo $this->getObjectVariable($this->filters, 'plan_nombre') ?>"></input>
		            </label>
		        </div>
				<div class="col-3">
		            <label>Imagen</label>
		            ...
		            <input type="text" class="form-control" name="plan_imagen" value="<?php echo $this->getObjectVariable($this->filters, 'plan_imagen') ?>"></input>
		            </label>
		        </div>
				<div class="col-3">
					<label>Ciudad</label>
	                <label class="input-group">
	                    ...
	                    <select class="form-control" name="plan_ciudad">
	                        <option value="">Todas</option>
	                        <?php foreach ($this->list_plan_ciudad as $key => $value) : ?>
	                            <option value="<?= $key; ?>" <?php if ($this->getObjectVariable($this->filters, 'plan_ciudad') ==  $key) { echo "selected";} ?>><?= $value; ?></option>
	                        <?php endforeach ?>
	                    </select>
	               </label>
	            </div>
	...
	<div class="content-dashboard">
		...
		<table class=" table table-striped  table-hover table-administrator text-left">
			...
			<tbody>
				...
				<?php $id =  $content->plan_id; ?>
					<tr>
						<td><?=$content->plan_nombre;?></td>
						<td>
							<?php if($content->plan_imagen) { ?>
								<img src="/images/<?= $content->plan_imagen; ?>"  class="img-thumbnail thumbnail-administrator" />
							<?php } ?>
							<div><?= $content->plan_imagen; ?></div>
						</td>
						<td><?= $this->list_plan_ciudad[$content->plan_ciudad];?>
						<td>
